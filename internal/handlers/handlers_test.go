...
var theTests = []struct {
	...
	// 	{key: "first_name", value: "Tomáš"},
	// 	{key: "last_name", value: "Tomšů"},
...
func TestRepository_Reservation(t *testing.T) {
	...
	// test case where reservation is not is session (reset everything)
	req, _ = http.NewRequest("GET", "/make-reservation", nil)
	ctx = getCtx(req)
	req = req.WithContext(ctx)
	rr = httptest.NewRecorder()
...
func TestRepository_PostReservation(t *testing.T) {
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("PostReservation handler failer when trying to fail inserting reservation: got %d, wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}
